...
http-response ^https:\/\/api\.zhihu\.com\/commercial_api\/app_float_layer script-path = Zhihu_remove_ads.js, requires-body = true, tag = 移除首页推广
http-response ^https:\/\/api\.zhihu\.com\/feed\/render\/tab\/config\? script-path = Zhihu_remove_ads.js, requires-body = true, tag = 移除首页推广
http-response ^https:\/\/api\.zhihu\.com\/(moments_v3|topstory\/hot-lists\/total|topstory\/recommend) script-path = Zhihu_remove_ads.js, requires-body = true, tag = 移除首页推广
http-response ^https:\/\/api\.zhihu\.com\/v2\/topstory\/hot-lists\/everyone-seeing\? script-path = Zhihu_remove_ads.js, requires-body = true, tag = 移除首页推广
http-response ^https:\/\/api\.zhihu\.com\/root\/tab script-path = Zhihu_remove_ads.js, requires-body = true, tag = 精简顶部标签

http-response ^https:\/\/(api|page-info)\.zhihu\.com\/(answers|articles)\/v2\/\d+ script-path = Zhihu_remove_ads.js, requires-body = true, tag = 移除首页推广

# 会员页面 - 弹窗、悬浮动图
http-response ^https:\/\/api\.zhihu\.com\/bazaar\/vip_tab\/header\? script-path = Zhihu_remove_ads.js, requires-body = true, tag = 移除会员页面推广

# 搜索页 - 关键词、猜你想搜
http-response ^https:\/\/api\.zhihu\.com\/search\/recommend_query\/v2\? script-path = zhihu_replace-body.js, requires-body = true, argument = ""recommend_queries":\{.+\}->"recommend_queries":{}", tag = 移除搜索页关键词

# 回答详情页 - 评论区顶部、下一个回答
http-response ^https:\/\/api\.zhihu\.com\/next-(bff|data|render)\? script-path = Zhihu_remove_ads.js, requires-body = true, tag = 移除回答详情页推广
http-response ^https:\/\/api\.zhihu\.com\/questions\/\d+(\/answers|\/feeds|\?include=) script-path = Zhihu_remove_ads.js, requires-body = true, tag = 移除回答详情页推广
http-response ^https:\/\/www\.zhihu\.com\/api\/v4\/(articles|answers)\/\d+\/recommendations?\? script-path = Zhihu_remove_ads.js, requires-body = true, tag = 移除回答详情页推广

# 其他 - 服务器推送配置
http-response ^https:\/\/appcloud2\.zhihu\.com\/v3\/config script-path = Zhihu_remove_ads.js, requires-body = true, tag = 拦截服务器推送配置
http-response ^https:\/\/m-cloud\.zhihu\.com\/api\/cloud\/config\/all\? script-path = Zhihu_remove_ads.js, requires-body = true, tag = 拦截服务器推送配置
